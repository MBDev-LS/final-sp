...
# Idea: Add 'BigHop' move option - allows the player to move N, E, S or W for two spaces
# Author: Louis
...
class Dastan:
	...
	def __CreateMoveOptionOffer(self):
		self._MoveOptionOffer.append('big hop')
	...
	def __CreateBigHop(self, Direction):
		NewMoveOption = MoveOption('big hop')
	...
	def __CreateMoveOption(self, Name, Direction):
		if Name == "big hop":
			return self.__CreateBigHop(Direction)
		if Name == "chowkidar":
	...
	def __CreateMoveOptions(self):
		...
		self._Players[0].AddToMoveOptionQueue(self.__CreateMoveOption("big hop", 1))
		...
		self._Players[1].AddToMoveOptionQueue(self.__CreateMoveOption("big hop", -1))
