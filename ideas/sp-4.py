...
# Idea: Allow users to set board dimensions at game start
...
def intInput(prompt: str) -> int:
	"""
	Display the prompt supplied when 
	the function is called and allows
	the user to enter an input. Repeats
...
def Main():
	rowNum = intInput('Enter the number of rows: ')
	colNum = intInput('Enter the number of columns: ')

	ThisGame = Dastan(rowNum, colNum, 4)
	ThisGame.PlayGame()
	print("Goodbye!")
	input()
